    ...
    class Function < Base
      ...
      def js_call(scope_chain, *params)
        arguments.each_with_index { |name, i|
          scope_chain[name.value] = params[i] || Jabl::RKelly::Runtime::UNDEFINED
        }
        function_visitor  = Jabl::RKelly::Visitors::FunctionVisitor.new(scope_chain)
        eval_visitor      = Jabl::RKelly::Visitors::EvaluationVisitor.new(scope_chain)
