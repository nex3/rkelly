    ...
    class Base
      ...
      def default_value(hint)
        ...
        when 'Number'
          ...
          if value_of.function || value_of.value.is_a?(Jabl::RKelly::JS::Function)
            return value_of
          end
          to_string = self['toString']
          if to_string.function || to_string.value.is_a?(Jabl::RKelly::JS::Function)
            return to_string
          end
        end
      end
      ...
      def unbound_method(name, object_id = nil, &block)
        ...
        unless Jabl::RKelly::JS::Base.instance_methods.include?(name.to_sym)
          Jabl::RKelly::JS::Base.class_eval do
            define_method(name, &block)
          end
        end
        Jabl::RKelly::JS::Base.instance_method(name)
      end
    end
  end
end
